...
std::shared_ptr<syntax_expr> function_table::infer_type(const std::string &func_name, syntax_fun_call &call)
{
    ...
    {
        bool match = false;
        std::string name = func_name.substr(1, func_name.size() - 1);
        if (call.parameters.size() != 1)
            throw inner_error(INNER_NO_MATCH_FUN, func_name);

        auto ptr_expr_param   = call.parameters[0];
        auto ptr_product_type = std::get_if<product_type>(&ptr_expr_param->type.type);
        auto ptr_sum_type     = std::get_if<sum_type>(&ptr_expr_param->type.type);
        // product_type
        if (ptr_product_type)
        {
        ...
        // sum_type
        else if(ptr_sum_type)
        {
            auto it_alters = ptr_sum_type->alters.begin();
            auto it_type = ptr_sum_type->types.begin();
            for(; it_alters != ptr_sum_type->alters.end();it_alters++,it_type++)
            {
                if(name == *it_alters)
                {
                    match = true;
