...
namespace yy {
#line 162 "src/parser.tab.hpp" // lalr1.cc:417
...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YYASSERT (tok == 0 || tok == token::LBRACE || tok == token::RBRACE);
      }
#else
      symbol_type (int tok)
        : super_type(token_type (tok))
      ...
      {
        YYASSERT (tok == token::ADDOP || tok == token::MULOP || tok == token::EXPOP);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      make_ADDOP (char v)
      {
        return symbol_type (token::ADDOP, std::move (v));
      }
#else
      static
      symbol_type
      make_ADDOP (const char& v)
      {
        return symbol_type (token::ADDOP, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      make_EXPOP (char v)
      {
        return symbol_type (token::EXPOP, std::move (v));
      }
#else
      static
      symbol_type
      make_EXPOP (const char& v)
      {
        return symbol_type (token::EXPOP, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
...
#line 1274 "src/parser.tab.hpp" // lalr1.cc:417
